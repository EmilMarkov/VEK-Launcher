...
impl ProviderDODI {
    ...
    async fn get_total_pages(&self) -> Result<u32, String> {
        let url = "https://www.1337xx.to/user/DODI/1";
    ...
    async fn process_page(&mut self, page: u32) -> Result<(), String> {
        let url = format!("https://www.1337xx.to/user/DODI/{}", page);
        match self.fetch_web_content(&url).await {
            Ok(data) => {
                ...
                for element in document.select(&title_selector) {
                    ...
                    let formatted_title = format_name(dodi_formatter(title));
...
#[tauri::command]
pub async fn get_torrent_info_dodi(url: String, window: Window) -> Result<(String, String), String> {
    let db = Arc::new(Mutex::new(Database::new().unwrap()));
    let torrent_service = Arc::new(Mutex::new(TorrentService::new(db.clone())));
    
    let provider = ProviderDODI::new(torrent_service);
